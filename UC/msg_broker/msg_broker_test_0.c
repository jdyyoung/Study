...
{
	if (!strcasecmp(msg_context->pszHost, IVS_MSG_TRIGGER)){

		if (!strcmp(msg_context->pszCmd, IVS_MSG_GET_FD_COUNT)){
			printf("[%s:%d]pszHost = %s,pszCmd=%s,pbyData=%d\n",__func__,__LINE__,msg_context->pszHost,msg_context->pszCmd,*((int*)msg_context->pbyData));
		} 
		else if (!strcmp(msg_context->pszCmd, IVS_MSG_GET_HBD_COUNT)){
			printf("[%s:%d]pszHost = %s,pszCmd=%s,pbyData=%d\n",__func__,__LINE__,msg_context->pszHost,msg_context->pszCmd,*((int*)msg_context->pbyData));
		}
...
static void srv_thread(void* param){
	...
	while(1){
		...
		if(!srv_flag){
			srv_flag = 1;
			MsgBroker_RegisterMsg(VENC_IVS_CMD_FIFO); 
			MsgBroker_Run(VENC_IVS_CMD_FIFO, msg_ivs_callback, NULL, &bTerminate);
...
static void cli_msg(const char* pszHost,const char* pszCmd,unsigned int value ){
		...
		if(MsgBroker_SendMsg(VENC_CMD_FIFO, &msg_context) == -1){
...
static void cli_thread(void* param){
	...
	while(1){
		...
		switch(rand_val){
			...
			break;
			case 1:
				cli_msg(IVS_HOST,IVS_MSG_MHBD,rand_val);
			break;
			case 2:
				cli_msg( "asc_host","asc_set",rand_val);
			break;
			case 3:
				cli_msg( "asc_host","asc_get",rand_val);
			break;
			default:
			break;
		}
