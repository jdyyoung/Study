...
作用：将文件a.txt(硬盘物理信息)映射到用户3G虚拟地址空间中的MMAP虚拟内存映射区中,一旦映射完毕将来访问用户的这块虚拟内存就是在访问对应	的物理地址(物理内存),不再经过内核的数据拷贝，加快文件的访问速度;第一个实参0:告诉内核,请在当前进程的3G虚拟地址空间的MMAP内存映射区中找一块空闲的虚拟内存用来映射文件
