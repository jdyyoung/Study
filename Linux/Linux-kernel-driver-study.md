...
4.linux内核并发和竞态
  ...
  先A进程来打开设备：
  	1.先加载 open_cnt = 1
  	由于linux内核支持单CPU进程与进程之前的抢占(高优先级的进程抢占低优先级进程的CPU资源)，
  	此时此刻，B进程抢占A进程的CPU资源
  B进程接着执行：
  	1.先加载 open_cnt = 1
  	2.修改,写回：open_cnt = 0;
  ...
  总结：产生问题的原因是进程与进程之前的抢占引起的竞争状态；
  	...
  	只要加载,修改,存储这三条语句在执行的时候,不会发生CPU资源的切换，
  	也就是让这三条语句的执行路径具有原子性，即可避免这种竞争问题
...
等待队列实现进程休眠的编程步骤：
	...
	5.让当前进程进入真正的休眠状态
	   schedule(); //永久性休眠
	   注意：
	   此时此刻代码执行到这里不再继续执行，等待事件得到满足被唤醒，或者接收到了信号
	   被唤醒(前提是可中断的休眠状态)
	6.一旦进程被唤醒,将从schedule函数返回，代码继续执行，然后要设置进程的状态为运行，并且将当前
	进程从休眠队列中移出
...
```
4.linux内核等待队列机制编程方法2：
  编程方法2最根本还是需要编程方法1(上述)来实现，只是内核将方法1中的某些步骤进行了封装，便于驱动开发者使用！
  ...
  编程步骤：
	...
	2.调用以下方法实现进程的休眠
	  ...
	  说明：
	  1.wq：表示定义初识化的等待队列头,休眠进程会添加到队列中去
	  2.conditon:
	  	如果为真,进程不休眠,立即返回
	  	如果为假,进程将在此休眠,休眠状态为不可中断,并且等待驱动主动唤醒，驱动主动唤醒时,驱动一定要
	  	将conditon置真
	  3.如果驱动主动唤醒,此宏返回,并且驱动还要将condtion置假,为下一次休眠做准备
	  参考代码：
	  int ispressed;
	  int xxx_read(...) //read
	  {
	  	wait_event(wq, ispressed);
	  	ispressed = 0; //置假
	  }	
	  
	  irqreturn_t xxx_isr(...) //中断处理函数
	  {
	  	ispressed = 1; //置真
	  	wake_up(&wq);
	  }  
	  或者：
	  wait_event_interruptible(wq, condtion);
	  1.wq：表示定义初识化的等待队列头,休眠进程会添加到队列中去
	  2.conditon:
	  	如果为真,进程不休眠,立即返回0
	  	如果为假,进程将在此休眠,休眠状态为可中断,并且等待驱动主动唤醒或者信号来唤醒，驱动主动唤醒时,驱
	  	动一定要将conditon置真
	  3.如果驱动主动唤醒,此宏返回,并且驱动还要将condtion置假,为下一次休眠做准备
	  参考代码：
	  int ispressed;
	  int xxx_read(...) //read
	  {
	  	wait_event_interruptible(wq, ispressed);	
	  	ispressed = 0; //置假
	  }	
