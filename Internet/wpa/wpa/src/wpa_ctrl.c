...
//for preventing file descriptor leakage
#ifndef O_CLOEXEC
...
struct wpa_ctrl * wpa_ctrl_open(const char *ctrl_path)
{
	...
	ctrl->s = socket(PF_UNIX, SOCK_DGRAM|SOCK_CLOEXEC, 0);
	if (ctrl->s < 0) {
		os_free(ctrl);
		return NULL;
	}
...
#ifdef CONFIG_CTRL_IFACE_UDP_IPV6
	ctrl->s = socket(PF_INET6, SOCK_DGRAM|SOCK_CLOEXEC, 0);
#else /* CONFIG_CTRL_IFACE_UDP_IPV6 */
	ctrl->s = socket(PF_INET, SOCK_DGRAM|SOCK_CLOEXEC, 0);
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
	if (ctrl->s < 0) {
		perror("socket");
		os_free(ctrl);
